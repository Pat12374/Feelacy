...
const policies = [
  ...
  { keywords: ["privacy", "data", "id photo"], label: "Privacy policy", href: "/legal/privacy", summary: "WineTreff limits personal-data use and does not retain delivery ID photographs by default." },
  { keywords: ["rule", "terms", "refund", "dispute", "alcohol", "safety"], label: "Marketplace terms", href: "/legal/terms", summary: "WineTreff facilitates marketplace transactions; independent sellers remain sellers of record and disputes or uncertain refund outcomes require human review." },
];
...
export async function runMarketplaceAssistant(input: { userId: string; message: string; mode?: AssistantMode; conversationId?: string }): Promise<AssistantReply> {
  ...
  if (mode === "BUYER") { results = await searchCatalog(input.message); fallback = results.length ? `I found ${results.length} currently available offer${results.length === 1 ? "" : "s"} matching your request. These results come directly from WineTreff's live catalog; confirm seller details on each listing.` : "I couldn't find a currently available offer matching those details. Try broadening the category, price, vintage, or producer."; }
  else if (mode === "SUPPORT") { const matches = policies.filter(p => p.keywords.some(k => q.includes(k))); const selected = matches.length ? matches : [policies[2]]; citations = selected.map(({label,href}) => ({label,href})); fallback = escalation ? "This needs human review. I can point you to the relevant WineTreff policy, but I won’t guess about legal, authenticity, dispute, or refund outcomes. Please contact WineTreff support." : selected.map(p => p.summary).join(" "); }
